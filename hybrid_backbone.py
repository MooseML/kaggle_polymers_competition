...
class GNN_Transformer_Hybrid(nn.Module):
    def __init__(self, gnn_dim, rdkit_dim, hidden_dim, extra_atom_dim, 
        ...
        super().__init__()
        act_map = {'ReLU': nn.ReLU(), 'ELU': nn.ELU(), 'LeakyReLU': nn.LeakyReLU(), 
                   'PReLU': nn.PReLU(), 'GELU': nn.GELU(), 'Swish': nn.SiLU()}
    ...
    def forward(self, data):
        ...
        # Last-resort sanitizer – replace any NaNs/Infs
        if has_nan(pad_out):
    ...
    def forward_backbone_only(self, data, return_gap: bool = False):
        ...
        # === NEW: True GAP over atom positions (exclude CLS) ===
    ...
    # def forward_backbone_only(self, data):
    #     """Forward pass through backbone only, returns CLS token output without MLP head"""
    #     # This is identical to forward() but stops at cls_out (before the MLP)
    ...
    #     return cls_out  # Return only the 512-d CLS token output 